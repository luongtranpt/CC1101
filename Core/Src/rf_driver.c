...
uint8_t GDO0_FLAG = 0; //Flag of the tx threshold interrupt. Used to control the reading and writting to the buffer and to check the
					   //arrival of a packet. Cờ ngắt ngướng TX, điều khiển việc đọc và ghi vào buffer và kiểm tra packet đến
...
-10, 0, 5, 7, 10 dBm for each carrier frequency using Wire-Wound Inductors
*/
...
const uint8_t patable_power_315[]  = {0x12,0x0D,0x1C,0x34,0x51,0x85,0xCB,0xC2};
const uint8_t patable_power_433[]  = {0x12,0x0E,0x1D,0x34,0x60,0x84,0xC8,0xC0};
const uint8_t patable_power_868[]  = {0x03,0x0F,0x1E,0x27,0x50,0x81,0xCB,0xC2};
const uint8_t patable_power_915[]  = {0x03,0x0E,0x1E,0x27,0x8E,0xCD,0xC7,0xC0};
//----------------------[REGISTER BASIC CONFIGURATION]------------------------
//Preset for Gaussian Frequency Shift Keying mod at 1.2KBits/s
const uint8_t cc1100_GFSK_1_2_kb[] = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration, assert when packet đc nhận bằng CRC OK, De-assert khi byte đầu tiên đc đọc từ RXFIFO 
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration, high impedance 3 state
                    0x80,  // IOCFG0        GDO0_Pin Output Pin Configuration, turm on tempurature sensor
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds, mức duy giảm RX:0dB, 33 byte TX FIRO, 32 byte RX FIFO
...
//                  0x3E,  // PKTLEN        Packet Length
                    ...
                    0x0C,  // PKTCTRL1      CRC_AUTOFLUSH enanble, 2 byte status đc thêm vào payload
					0x45,  // PKTCTRL0      Packet Automation Control, Turn on WHITE DATA, normal mode use FIFO for TX and RX, Enable CRC, various length packet
                    0xFF,  // ADDR          Device Address
                    0x00,  // CHANNR        Channel Number
                    0x08,  // FSCTRL1       Frequency Synthesizer Control, Iif = 203kHz
                    0x00,  // FSCTRL0       Frequency Synthesizer Control
                    0x21,  // FREQ2         Frequency Control Word, High Byte, defalt 868Mhz
                    0x65,  // FREQ1         Frequency Control Word, Middle Byte
                    0x6A,  // FREQ0         Frequency Control Word, Low Byte
                    0xF5,  // MDMCFG4       Modem Configuration, Data rate 1.2kBaud
                    0x83,  // MDMCFG3       Modem Configuration, Data rate 1.2kBaud
                    0x13,  // MDMCFG2       Modem Configuration, GSFK, Enable Manchester, sync world 30/32
                    0xC0,  // MDMCFG1       Modem Configuration, Enable sửa lỗi FEC, số bit preemble 4(010)
                    0xF8,  // MDMCFG0       Modem Configuration, spacing channel 200k
                    0x15,  // DEVIATN       Modem Deviation Setting
                    0x07,  // MCSM2         Main Radio Control State Machine Configuration
                    0x00,  // MCSM1         Main Radio Control State Machine Configuration //TODO was 0x0C
                //   0x3C,  // MCSM1        CCA when RSSI below threshold an unless đang nhận packet, RX node
				//	0x32,  // MCSM1         CCA when RSSI below threshold an unless đang nhận packet, TX node
...
FRAMESTATUS_TypeDef receive_frame_test(uint8_t* frame_buffer, int len)
{
	rf_receive();
...
//	rf_write_register(PKTLEN, (frame_len)%256);
	...
	pktcrtl0 = pktcrtl0 | 0x2;
	rf_write_register(PKTCTRL0, pktcrtl0);
	if(!polling_while_lower(RXBYTES, len)) return TIMEOUT; //TODO reconfigure RX Threshold to detect the first bytes??
	
	
	rf_read_data(RXFIFO, frame_buffer, len);
//	for(int j = 0; j<20;j++)
//	{
//	c[j]= rf_read_register(0x80+j);
//	b[j] = rf_read_register(0xC0+j);
//	}
	...
	uint8_t   rssi_dec = rf_read_register(RSSI);

    if(rssi_dec >= 128)
        helrssi = (uint8_t)((uint8_t)(rssi_dec - 256) / 2) - rssi_offset;
    else
        helrssi = (rssi_dec / 2) - rssi_offset;
...
FRAMESTATUS_TypeDef receive_frame(uint8_t* frame_buffer, uint16_t* len, uint8_t data_len_loc, uint8_t data_len_size, uint8_t* lqi, uint8_t* rssi){
	...
	*len = 0;
	uint16_t frame_len;
	uint8_t data_field_size = sizeof(*len); //2 byte
	...
	uint8_t SFD = 0x08; //Sync Word OK? Addr (if enabled) OK?
	...
	*/
	...
	//get frame size, wait until has first byte
...
//	rf_read_data(RXFIFO, frame_buffer, data_len_loc+data_field_size); //read 2 byte dau tien 
	
  if(!polling_while_lower(RXBYTES, data_len_loc+data_len_size)) return TIMEOUT; //TODO reconfigure RX Threshold to detect the first bytes??
	
	rf_read_data(RXFIFO, frame_buffer, data_len_loc+data_len_size); //read 2 byte dau tien 

	
//	//first len is length of packet, function return it: 
//	frame_len = _get_frame_size(frame_buffer, data_len_loc, data_len_size); 
//	check3 = (uint8_t)frame_len;
//	*len +=data_len_loc+data_field_size; // 2 byte
//	if(frame_len > max_len) frame_len = max_len;
//	//printf("FRAME LEN: %d\n\r", frame_len);


//	uint16_t remaining_len = frame_len-*len;// length data = length packet - 2 byte dau
//	//set packet length
//	rf_write_register(PKTLEN, (frame_len)%256);
...
void rf_read_fifotx(uint8_t addr, uint8_t* data, uint8_t size){
	/**
	 * @brief Reads multiple data.
	 * @param addr Base address.
	 * @param data The buffer where the read data will be stored.
